...
def refactor_expression_script(code, kind, symbols_map):
    ...
    uid_reverse_map = {
        did[-1].id: sym for did, sym in symbols_map.descid_to_symbol.iteritems()
    }
    ...
    code = refactor.indentation(
        code, "  "
    )  # To match the indentation of the plugin stub.
    ...
    if kind == "ObjectData":
        ...
        code, exec_code = refactor.split_and_refactor_global_function(
            ...
            "main",
            "Execute",
            ["self", "op", "doc", "host", "bt", "priority", "flags"],
...
class SymbolMap(object):
    ...
    def allocate_symbol(self, node):
        ...
        if (
            node["descid"][-1].dtype == c4d.DTYPE_SEPARATOR
            and not node["bc"][c4d.DESC_NAME]
        ):
...
class Converter(object):
    ...
    def autofill(self, default_plugin_name="My Plugin"):
        ...
        if other.link:
            other.read_from_link()

        if not self.plugin_name:
            self.plugin_name = other.plugin_name or (
                self.link.GetName() if self.link else ""
            )
        if not self.plugin_name:
        ...
        if not self.resource_name:
            ...
            self.resource_name = (
                self.plugin_type_info().get("resprefix", "") + self.resource_name
            )
        ...
        if not self.symbol_prefix:
            self.symbol_prefix = (
                re.sub("[^\w\d]+", "_", self.plugin_name).rstrip("_").upper() + "_"
            )
            ...
            result.update(
                {
                    ...
                    "strings_us": j(
                        "res", "strings_us", "description", f("{self.resource_name}.str")
                    ),
                }
            )
    ...
    def create(self):
        ...
        ud_main_group = next(
            (x for x in ud_tree.children if x["descid"] == c4d.DescID(c4d.ID_USERDATA))
        )
        ud_tree.visit(
            lambda x: symbol_map.allocate_symbol(x) if x != ud_main_group else None, False
        )
        ...
        with open(files["header"], "w") as fp:
            ...
            if self.plugin_id:
                fp.write(
                    self.indent
                    + "{self.resource_name} = {self.plugin_id},\n".format(self=self)
                )
        ...
        with open(files["description"], "w") as fp:
            ...
            for base, propgroup in [
                ("Obase", "ID_OBJECTPROPERTIES"),
                ("Tbase", "ID_TAGPROPERTIES"),
                ("Xbase", "ID_SHADERPROPERTIES"),
                ("Mbase", "ID_MATERIALPROPERTIES"),
            ]:
            ...
            for node in ud_tree.children:
                if node["descid"] == c4d.DescID(c4d.ID_USERDATA):
                    continue
        ...
        with open(files["strings_us"], "w") as fp:
            ...
            fp.write(
                '{self.indent}{self.resource_name} "{self.plugin_name}";\n'.format(
                    self=self
                )
            )
        ...
        if "plugin" in files and (self.overwrite or not os.path.isfile(files["plugin"])):
            ...
            if code_parts.get("member_code"):
                ...
                code_parts["member_code"] = "\n".join(
                    "  " + l for l in code_parts["member_code"].split("\n")
                )
            ...
            context = {
                ...
                "parameters": [
                    ...
                    for did, bc in ud
                    if did in symbol_map.descid_to_node
                ],
                ...
                "plugin_icon": "res/icons/" + os.path.basename(files["icon"])
                if files.get("icon")
                else None,
    ...
    def render_parameter(self, fp, node, symbol_map, depth=1):
        ...
        if dtype == c4d.DTYPE_GROUP:
            ...
            if dtype == c4d.DTYPE_BOOL:
                ...
                multiplier = (
                    100
                    if (not has_cycle and bc[c4d.DESC_UNIT] == c4d.DESC_UNIT_PERCENT)
                    else 1
                )
                ...
                if has_cycle:
                    ...
                    if default_name:
                        props.append(
                            "DEFAULT {};".format(
                                symbol_map.get_cycle_symbol(node, default_name)
                            )
                        )
                ...
                # c4d.CUSTOMGUI_QUICKTAB)
                    ...
                    symbol_map.add_hardcoded_description(
                        node, "c4d.DESC_CUSTOMGUI", 200000281
                    )
                # RadioButtons CustomGUI.
                elif bc[c4d.DESC_CUSTOMGUI] == 1019603:
                    symbol_map.add_hardcoded_description(
                        node, "c4d.DESC_CUSTOMGUI", 1019603
                    )
                elif bc[c4d.DESC_CUSTOMGUI] in (
                    c4d.CUSTOMGUI_REAL,
                    c4d.CUSTOMGUI_LONG,
                    c4d.CUSTOMGUI_CYCLE,
                ):
                    ...
                    print(
                        "Note: unknown customgui:",
                        bc[c4d.DESC_NAME],
                        bc[c4d.DESC_CUSTOMGUI],
                    )

                if not has_cycle:
                    if bc.GetType(c4d.DESC_MIN) == dtype and not is_minvalue(
                        bc[c4d.DESC_MIN]
                    ):
                        props.append("MIN {};".format(bc[c4d.DESC_MIN] * multiplier))
                    if bc.GetType(c4d.DESC_MAX) == dtype and not is_maxvalue(
                        bc[c4d.DESC_MAX]
                    ):
                        props.append("MAX {};".format(bc[c4d.DESC_MAX] * multiplier))

                    if bc[c4d.DESC_CUSTOMGUI] in (
                        c4d.CUSTOMGUI_LONGSLIDER,
                        c4d.CUSTOMGUI_REALSLIDER,
                        c4d.CUSTOMGUI_REALSLIDERONLY,
                    ):
                        if bc.GetType(c4d.DESC_MINSLIDER) == dtype:
                            props.append(
                                "MINSLIDER {};".format(
                                    bc[c4d.DESC_MINSLIDER] * multiplier
                                )
                            )
                        if bc.GetType(c4d.DESC_MAXSLIDER) == dtype:
                            props.append(
                                "MAXSLIDER {};".format(
                                    bc[c4d.DESC_MAXSLIDER] * multiplier
                                )
                            )
                ...
                if dtype == c4d.DTYPE_VECTOR:
                    if bc.GetType(c4d.DESC_MIN) == c4d.DTYPE_VECTOR and not is_minvalue(
                        bc[c4d.DESC_MIN]
                    ):
                        props.append(
                            vecprop("MIN", bc.GetVector(c4d.DESC_MIN) * multiplier)
                        )
                    if bc.GetType(c4d.DESC_MAX) == c4d.DTYPE_VECTOR and not is_maxvalue(
                        bc[c4d.DESC_MAX]
                    ):
                        props.append(
                            vecprop("MAX", bc.GetVector(c4d.DESC_MAX) * multiplier)
                        )
                ...
                print(
                    "Unhandled datatype:", dtype, "({})".format(node["bc"][c4d.DESC_NAME])
                )
                return
    ...
    def render_symbol_string(self, fp, node, symbol_map):
        ...
        for item_id, name in cycle or []:
            ...
            fp.write(
                self.indent * 2
                + '{} "{}";\n'.format(symbol_map.get_cycle_symbol(node, name), strname)
            )
...
class PrototypeConverter(nr.c4d.ui.Component):
    def render(self, dialog):
        ...
        self["clear_plugin_name"].add_event_listener(
            "click", self._clear_field("plugin_name")
        )
        self["clear_plugin_id"].add_event_listener(
            "click", self._clear_field("plugin_id")
        )
        self["clear_resource_name"].add_event_listener(
            "click", self._clear_field("resource_name")
        )
        self["clear_symbol_prefix"].add_event_listener(
            "click", self._clear_field("symbol_prefix")
        )
        self["clear_icon_file"].add_event_listener(
            "click", self._clear_field("icon_file")
        )
        self["clear_plugin_directory"].add_event_listener(
            "click", self._clear_field("plugin_directory")
        )
