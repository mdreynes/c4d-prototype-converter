...
def get_library_scripts():
    dirs = [
        os.path.join(c4d.storage.GeGetC4DPath(x), "scripts")
    ...
    def recurse(directory, depth=0):
        ...
        for name in os.listdir(directory):
            ...
            if name.endswith(".py"):
                name = (
                    "    " * depth + name + "&i{}&".format(c4d.RESOURCEIMAGE_PYTHONSCRIPT)
                )
...
class Converter(object):
        ...
        if not self.plugin_name:
            if self.script_file:
                ...
                if metadata.get("name"):
                    ...
                    self.plugin_name = os.path.splitext(
    ...
    def files(self):
        ...
        if self.icon_file:
            ...
            result["icon"] = os.path.join(
                parent_dir, "res/icons/{0}{1}".format(plugin_filename, suffix)
            )
